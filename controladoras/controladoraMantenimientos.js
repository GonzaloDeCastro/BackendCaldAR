...
exports.addNewMantenimiento = async (req, res) => {
  try {
    ...
    let mantenimientoJSON = fs.readFileSync('datos/mantenimientos.json', 'utf8'); //data from JSON file
    ...
    if (!tecnico) {
        return res.status(400).send({ error: 'No ingreso el nombre del Técnico responsable.' });
    }   
    if (!cliente) {
        return res.status(400).send({ error: 'No ingreso el dni del Cliente.' });
    }
    if (!fecha) {
        return res.status(400).send({ error: 'No ingreso la Fecha del Mantenimiento.' });
    } 
    if (!caldera) {
        return res.status(400).send({ error: 'No selecciono el tipo de Caldera.' });
    } 
    if (!tipo_mantenimiento) {
        return res.status(400).send({ error: 'No selecciono el tipo de Mantenimiento.' });
    }    

    const mantenimientoId = Number(mantenimientos[mantenimientos.length - 1].id) + 1;
    const newMantenimiento = { id: mantenimientoId, tecnico, cliente, fecha, caldera, tipo_mantenimiento };
    mantenimientos.push(newMantenimiento);

    fs.writeFileSync('datos/mantenimientos.json', JSON.stringify(mantenimientos), {
        encoding: 'utf8',
        flag: 'w',
    });
...
exports.getAllMantenimientos = async (req, res) => {
    try {
        let mantenimientoJSON = fs.readFileSync('datos/mantenimientos.json', 'utf8');
        let mantenimientos = JSON.parse(mantenimientoJSON);
...
exports.getMantenimientoByTipo = async (req, res) => {
    try {
        ...
        if (mantenimiento.length === 0)
            return res.status(400).json(`No se encontraron Mantenimientos del tipo ${mantenimientoTipo}`);
...
exports.updateMantenimiento = async (req, res) => {
    try {
        ...
        const { id, tecnico, cliente, fecha, caldera, tipo_mantenimiento } = req.body;
        ...
        if (mantenimientoIndex === -1) {
            return res.status(400).send({ error: `No existe el Mantenimiento con id: ${mantenimientoId}.` });
        }
        if (!tecnico) {
            return res.status(400).send({ error: 'No ingreso el nombre del Técnico responsable.' });
        }   
