...
exports.addNewcliente = async (req, res) => {
  try {
    ...
    if (!direccion) {
      return res.status(400).send({ error: 'No ingreso direccion de la cliente.' });
    }

    if (!tipo) {
      return res.status(400).send({ error: 'No especifico el tipo de cliente.' });
    }
...
exports.getclienteByTipo = async (req, res) => {
  try {
    ...
    let cliente = clientes.filter(
      (cliente) => cliente.tipo === clienteTipo
    );

    if (cliente.length === 0)
      return res.status(400).json(`No se encontraron clientes del tipo ${clienteTipo}`);
...
exports.updatecliente = async (req, res) => {
  try {
    ...
    if (!direccion) {
      return res.status(400).send({ error: 'No ingreso descripcion de la cliente.' });
    }
